...
import autoencoders.sae.model as ae
import numpy as np
import torch
from autoencoders.sae.sae import StackedAutoEncoder
...
class SAETransformerBase(TransformerMixin, BaseEstimator):
    def __init__(
        self,
        dimensions: List[int],
...
class SAETransformer(SAETransformerBase):
...
class SAERepresentationTransformer(SAETransformerBase):
    def transform(self, X):
        if self.autoencoder is None:
            raise NotFittedError
        if issparse(X):
